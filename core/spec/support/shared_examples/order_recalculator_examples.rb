...
RSpec.shared_examples "an order recalculator" do
  ...
  describe "promotion recalculation" do
    it "calls the Promotion Adjustments Recalculator" do
      ...
      expect(adjuster).to receive(:call).with(persist: true)
      updater.recalculate
    end
  end
  ...
  context "completed order" do
    ...
    it "updates payment state" do
      ...
      expect { updater.recalculate }.to change { order.payment_state }.from(nil)
    end
    ...
    context "with a shipment" do
      ...
      it "updates each shipment" do
        ...
        updater.recalculate
        expect(shipment).to have_received(:recalculate_state).at_least(:once)
      end

      it "updates the shipment amount" do
        allow(shipment).to receive(:assign_amounts)
        allow(shipment).to receive(:persist_amounts)
        updater.recalculate
        expect(shipment).to have_received(:assign_amounts).at_least(:once)
        expect(shipment).to have_received(:persist_amounts).at_least(:once)
      end
    end
  end
  ...
  context "incompleted order" do
    ...
    it "doesnt update payment state" do
      order.payment_state = nil
      expect { updater.recalculate }.not_to change { order.payment_state }.from(nil)
    end

    it "doesnt update shipment state" do
      order.shipment_state = nil
      expect { updater.recalculate }.not_to change { order.shipment_state }.from(nil)
    end
  end
